...
public class ServiceTransaction implements IService<Transaction> {
    ...
    public void modifier(Transaction transaction) throws SQLException {
        String query = "UPDATE transaction SET montant_total = ?, mode_paiement = ?, date_paiement = ? WHERE id_transaction = ?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setDouble(1, transaction.getMontant_total());
            stmt.setString(2, transaction.getMode_paiement());
            stmt.setTimestamp(3, transaction.getDate_paiement());
            stmt.setInt(4, transaction.getId_transaction());
            stmt.executeUpdate();
        }
    }
    ...
    private boolean utilisateurExiste(int utilisateurId) throws SQLException {
        String query = "SELECT COUNT(*) FROM utilisateur WHERE id_utilisateur = ?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
